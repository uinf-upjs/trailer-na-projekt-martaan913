        ...
        setContent {
            ...
            HomeworkPlannerTheme(darkTheme = isDark) {
                Surface(
                    ...
                    HomeworkScene(themeViewModel, viewModel)
                }
            }
        }
    }
...
fun HomeworkScene(
    ...
    Column (
        ...
        LazyColumn{items(homeworkList.value){item ->
...
fun HomeworkCard(
    ...
    Surface(
        ...
        Column(
            ...
            Text(text = "${stringResource(id = R.string.subject_label)}: ${homework.subject}")
            Text(text = "${stringResource(id = R.string.title_label)}: ${homework.title}")
            if (homework.description!!.length > 26){
                Text(text = "${stringResource(id = R.string.description_label)}: ${homework.description?.substring(0,25)}")
            }else{
                Text(text = "${stringResource(id = R.string.description_label)}: ${homework.description}")
            }
            Text(text = "${stringResource(id = R.string.deadline_label)}: ${homework.deadline}")
        }
    }
    if (showDialogDelete) {
        AlertDialog(
